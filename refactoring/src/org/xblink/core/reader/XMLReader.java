package org.xblink.core.reader;
...
import org.xblink.core.Constants;
import org.xblink.core.XMLObject;
import org.xblink.core.XRoot;
import org.xblink.core.transfer.ClassLoaderSwitcher;
import org.xblink.core.transfer.ImplClasses;
import org.xblink.core.transfer.ReferenceObject;
...
public class XMLReader {
	 ...
	 *            DOM驱动
	 */
	public Object readXML(String filePath, Class<?> clz, ClassLoader classLoader,Adapter adapter) {
	...
	public Object readXML(String filePath, Class<?> clz, Class<?>[] implClasses,
		...
		try {
			return readStart(new BufferedInputStream(new FileInputStream(new File(filePath))), clz,
					implClasses, classLoader,  adapter);
		} catch (Exception e) {
	 ...
	 *            Java类对象
	 */
	public Object readXML(InputStream inputStream, Class<?> clz, ClassLoader classLoader,Adapter adapter) {
		return readXML(inputStream, clz, new Class<?>[] {}, classLoader,  adapter);
	}
	...
	public Object readXML(InputStream inputStream, Class<?> clz, Class<?>[] implClasses,
			ClassLoader classLoader,Adapter adapter) {
		try {
			return readStart(inputStream, clz, implClasses, classLoader,  adapter);
		} catch (Exception e) {
			throw new RuntimeException(Constants.DESERIALIZE_FAIL, e);
		}
	}
	...
	private Object readStart(InputStream in, Class<?> clz, Class<?>[] implClasses,
			ClassLoader classLoader,Adapter adapter) throws Exception {
		// 解析过的对象，方便其他对象引用
		...
		transferInfo.setXmlAdapter(adapter);
		// XML类对象反序列工具类初始化
		XMLObjectReader xmlObjectRead = new XMLObjectReader();
		try {
			XMLDocument document = xmlAdapter.setDocument(in);
